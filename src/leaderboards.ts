...
function editLeaderboard(plr: Player, leaderboard: Entity, loc: Vector3) {
    new ModalFormData()
    ...
    .show(plr).then(response => {
        ...
        const [ obj, disp, numCo, namCo, scoCo, top, del ] = response.formValues as [ string, string, string, string, string, number, boolean ]

        if (!leaderboard) leaderboard = plr.dimension.spawnEntity('floating:text', loc);
        if (del) return leaderboard.remove()
...
const objDB: { [key: string]: ScoreboardObjective } = {};

function getScore(target: Entity | string | ScoreboardIdentity, objective: string) {
	try {
		return (objDB[objective] ??= world.scoreboard.getObjective(objective))?.getScore(target) ?? 0;
	} catch { return 0; }
};

function getStringProperty(entity: Entity, id: string): string {
    return entity?.getDynamicProperty(id) as string
}

function getNumberProperty(entity: Entity, id: string): number {
    return entity?.getDynamicProperty(id) as number
}

function topleaderboard(obj: string) {
